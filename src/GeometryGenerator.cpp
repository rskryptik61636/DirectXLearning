...
{
	using namespace DirectX;	// adding to ensure all DirectXMath types are accessible (Rohit S, 2014-02-10)
...
void GeometryGenerator::Subdivide(MeshData& meshData)
{
	...
	for(UINT i = 0; i < numTris; ++i)
	{
		...
		m0.Position = DXVector3(
			0.5f*(v0.Position.x + v1.Position.x),
			0.5f*(v0.Position.y + v1.Position.y),
			0.5f*(v0.Position.z + v1.Position.z));

		m1.Position = DXVector3(
			0.5f*(v1.Position.x + v2.Position.x),
			0.5f*(v1.Position.y + v2.Position.y),
			0.5f*(v1.Position.z + v2.Position.z));

		m2.Position = DXVector3(
...
void GeometryGenerator::CreateGeosphere(float radius, UINT numSubdivisions, MeshData& meshData)
{
	...
	DXVector3 pos[12] = 
	{
		DXVector3(-X, 0.0f, Z),  DXVector3(X, 0.0f, Z),  
		DXVector3(-X, 0.0f, -Z), DXVector3(X, 0.0f, -Z),    
		DXVector3(0.0f, Z, X),   DXVector3(0.0f, Z, -X), 
		DXVector3(0.0f, -Z, X),  DXVector3(0.0f, -Z, -X),    
		DXVector3(Z, X, 0.0f),   DXVector3(-Z, X, 0.0f), 
		DXVector3(Z, -X, 0.0f),  DXVector3(-Z, -X, 0.0f)
	};
...
void GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, UINT sliceCount, UINT stackCount, MeshData& meshData)
{
	...
	for(UINT i = 0; i < ringCount; ++i)
	{
		...
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			vertex.Position = DXVector3(r*c, y, r*s);
			...
			// This is unit length.
			vertex.TangentU = DXVector3(-s, 0.0f, c);

			float dr = bottomRadius-topRadius;
			DXVector3 bitangent(dr*c, -height, dr*s);
...
void GeometryGenerator::CreateGrid(float width, float depth, UINT m, UINT n, MeshData& meshData)
{
	...
	for(UINT i = 0; i < m; ++i)
	{
		...
		for(UINT j = 0; j < n; ++j)
		{
			...
			meshData.Vertices[i*n+j].Position = DXVector3(x, 0.0f, z);
			meshData.Vertices[i*n+j].Normal   = DXVector3(0.0f, 1.0f, 0.0f);
			meshData.Vertices[i*n+j].TangentU = DXVector3(1.0f, 0.0f, 0.0f);
